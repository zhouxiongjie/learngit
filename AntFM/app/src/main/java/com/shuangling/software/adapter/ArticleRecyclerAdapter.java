...
public class ArticleRecyclerAdapter extends RecyclerView.Adapter implements View.OnClickListener {
    ...
    private final Map<Integer, TypeAndData> mPositionTypeMap = new LinkedHashMap<>();
...
//        if (loadTimes >= 3)
    ...
    public interface PostItemClickListener {
        void onItemClick(ColumnContent content);
    }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NotNull ViewGroup parent, int viewType) {
        ...
        } else if (viewType == TYPE_NO_COMMENT) {//无评论
            return new NoCommentViewHolder(inflater.inflate(R.layout.no_data_layout, parent, false));//需要延迟加载
    ...
    public void onBindViewHolder(@NotNull RecyclerView.ViewHolder holder, final int position) {
        ...
        if (type == TYPE_HEAD) {
            ...
            if (content.getAlbums().getStatus() == 1) { //已完结
            ...
            albumViewHolder.count.setText(String.format(Locale.CHINESE, "%d集", content.getAlbums().getCount()));
...
//                            Log.i("test","getLineCount>2");
            ...
            videoViewHolder.playTimes.setText(String.format(Locale.CHINESE, "%d次播放", content.getView()));
            ...
            gallerieOneViewHolder.commentNum.setText(String.format(Locale.CHINESE, "%d评论", content.getComment()));
            gallerieOneViewHolder.count.setText(String.format(Locale.CHINESE, "%d图", content.getGallerie().getCount()));
            ...
            gallerieViewThreeHolder.commentNum.setText(String.format(Locale.CHINESE, "%d评论", content.getComment()));
            gallerieViewThreeHolder.count.setText(String.format(Locale.CHINESE, "%d图", content.getGallerie().getCount()));
...
//                                Log.i("test", "layout.getWidth()<title.getWidth()");
            ...
            specialViewHolder.commentNum.setText(String.format(Locale.CHINESE, "%d评论", content.getComment()));
            ...
            if (mTotalComments == 0) {
                ...
                viewHolder.commentNum.setText(String.format(Locale.CHINESE, "全部评论(%d)", mTotalComments));
            }
...
//可省略
            ...
            if (comment.getComment_count() > 0) {
                vh.reply.setText(String.format(Locale.CHINESE, "%d回复", comment.getComment_count()));
            ...
            if (((Comment) mPositionTypeMap.get(position).parentData).isExpand()) {
                ...
                if (((Comment) mPositionTypeMap.get(position).parentData).getParent_comment().size() > DEFAULT_SHOW_CHILD_NUMBER && ((Comment) mPositionTypeMap.get(position).parentData).getParent_comment().get(DEFAULT_SHOW_CHILD_NUMBER - 1).getId() == comment.getId()) {
                    vh.showMore.setText(String.format(Locale.CHINESE, "查看全部%d条回复", ((Comment) mPositionTypeMap.get(position).parentData).getParent_comment().size()));
...
//音频详情头 +1
    ...
    public int getItemViewType(int position) {
        return mPositionTypeMap.get(position).type;
    }

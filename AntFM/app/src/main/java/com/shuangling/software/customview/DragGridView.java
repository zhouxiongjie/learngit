...
public class DragGridView extends GridView {
    ...
    public TextView getEditorTextView() {
        return mEditorTextView;
    }

    public void setEditorTextView(TextView editorTextView) {
        this.mEditorTextView = editorTextView;
    }
	...
	public DragGridView(Context context, AttributeSet attrs, int defStyle) {
		...
		mStatusHeight = CommonUtils.getStatusBarHeight(context); //获取状态栏的高度
	}
	...
	private Runnable mLongClickRunnable = new Runnable() {
		...
		public void run() {
			if(mStartDragItemView!=null){
				isDrag = true; //设置可以拖拽
				((MyColumnGridViewAdapter)getAdapter()).setIsEditor(true);
				getEditorTextView().setText("完成");
				mVibrator.vibrate(50); //震动一下
				mStartDragItemView.setVisibility(View.INVISIBLE);//隐藏该item
				getParent().requestDisallowInterceptTouchEvent(true);
				Log.i("test", "requestDisallowInterceptTouchEvent(true);");
				//根据我们按下的点显示item镜像
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		Log.i("test", "dispatchTouchEvent" +ev.getAction());
		switch(ev.getAction()){
		case MotionEvent.ACTION_DOWN:
			...
			if(mDragPosition == AdapterView.INVALID_POSITION||mDragPosition==0||mDragPosition==1){
			...
			if(mStartDragItemView==null){
				Log.i("test", "mStartDragItemView=null");
			}
...
//			mHandler.removeCallbacks(mScrollRunnable);
	...
	public boolean onTouchEvent(MotionEvent ev) {
		Log.i("test", "onTouchEvent" +ev.getAction());
...
*/	
	...
	private void onSwapItem(int moveX, int moveY){
		...
		if(tempPosition!=0&&tempPosition!=1&&tempPosition != mDragPosition && tempPosition != AdapterView.INVALID_POSITION){

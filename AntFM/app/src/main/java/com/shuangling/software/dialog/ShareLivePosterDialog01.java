 ...
 */
public class ShareLivePosterDialog01 extends BaseCircleDialog {
    ...
    Unbinder unbinder1;
    private LiveRoomInfo01.RoomInfoBean mLiveRoomInfo;
    private String mUrl;

    public interface ShareHandler {
        void onShare(String platform, Bitmap bitmap);

        void onShare(String platform, LiveRoomInfo01.RoomInfoBean liveRoom);
    ...
    public LiveRoomInfo01.RoomInfoBean getLiveRoomInfo() {
        return mLiveRoomInfo;
    }

    public void setLiveRoomInfo(LiveRoomInfo01.RoomInfoBean liveRoomInfo) {
    ...
    public static ShareLivePosterDialog01 getInstance(LiveRoomInfo01.RoomInfoBean liveRoomInfo, String url) {
        ShareLivePosterDialog01 dialogFragment = new ShareLivePosterDialog01();
...
//mLayoutPoster
        ...
        switch (view.getId()) {
            ...
            case R.id.weiXin:
                if (mShareHandler != null) {
                    mShareHandler.onShare(Wechat.NAME, mLiveRoomInfo);
                }
                dismiss();
                break;
            case R.id.weiXinFriends:
                if (mShareHandler != null) {
                    mShareHandler.onShare(WechatMoments.NAME, mLiveRoomInfo);
                }
                dismiss();
                break;
            case R.id.weiXinCollect:
                if (mShareHandler != null) {
                    mShareHandler.onShare(WechatFavorite.NAME, mLiveRoomInfo);
                }
                dismiss();
                break;
            case R.id.weibo:
                if (mShareHandler != null) {
                    mShareHandler.onShare(SinaWeibo.NAME, mLiveRoomInfo);
                }
                dismiss();
                break;
            case R.id.qq:
                if (mShareHandler != null) {
                    mShareHandler.onShare(QQ.NAME, mLiveRoomInfo);
                }
                dismiss();
                break;
            case R.id.qqZone:

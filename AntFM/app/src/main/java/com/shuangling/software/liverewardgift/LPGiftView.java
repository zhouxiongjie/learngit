...
public class LPGiftView extends RelativeLayout {
    ...
    private  void init(){
        ...
        final SimpleDraweeView giftIcon = (SimpleDraweeView) findViewById(R.id.ivgift);
        final RelativeLayout giftTextContainerLayout = (RelativeLayout) findViewById(R.id.rlparent);
        SimpleDraweeView userIcon = (SimpleDraweeView) findViewById(R.id.user_icon);
        if (!TextUtils.isEmpty(mAnimMessage.getHeadUrl())) {
            Uri uri = Uri.parse(mAnimMessage.getHeadUrl());
            int width = CommonUtils.dip2px(38);
            int height = width;
            ImageLoader.showThumb(uri, userIcon, width, height);
        }
        ...
        if (!TextUtils.isEmpty(mAnimMessage.getGiftUrl())) {
            Uri uri = Uri.parse(mAnimMessage.getGiftUrl());
            int width = CommonUtils.dip2px(80);
            int height = width;
            ImageLoader.showThumb(uri, giftIcon, width, height);
        }

...
public class LayoutManger extends LinearLayoutManager implements RecyclerView.OnChildAttachStateChangeListener {
    ...
    public LayoutManger(Context context) {
        super(context);
    }

    public LayoutManger(Context context, int orientation, boolean reverseLayout) {
...
//        }
        ...
        if (onViewPagerListener != null) {
            onViewPagerListener.onPageSelected(getPosition(view), getItemCount());
        }
    }
        ...
        switch (state) {
            case RecyclerView.SCROLL_STATE_IDLE:// item 移动停止后 执行
                ...
                if (onViewPagerListener != null) {
                    ...
                    onViewPagerListener.onPageSelected(position, getItemCount());
